...
function Calendar(data, svgRef, {
  ...
  function pathMonth(t) {
    ...
    const d = Math.max(0, Math.min(weekDays, countDay(datetime.getUTCDay())));
    const w = timeWeek.count(d3.utcYear(datetime), datetime);
  ...
  const cell = year.append("g")
    ...
    .data(weekday === "weekday"
      ? ([, I]) => I.filter(i => ![0, 6].includes(new Date(X[i]).getUTCDay()))
      : ([, I]) => I)
    .join("rect")
  ...
  const month = year.append("g")
    .selectAll("g")
    .data(([, I]) => d3.utcMonths(d3.utcMonth(new Date(X[I[0]])), new Date(X[I[I.length - 1]])))
    .join("g");
...
const CalendarComponent = ({ data, config }) => {
  const svgRef = useRef(null);

  useEffect(() => {
    Calendar(data, svgRef, config);
  }, [data, config])
